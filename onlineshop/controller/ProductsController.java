...
public class ProductsController {
    ...
    public Product updateProduct(@RequestBody Product newProduct) {
        Product result = productList.stream()
                .filter(product -> product.getProductID() == newProduct.getProductID())
                .findFirst()
                .orElse(null);
        if (result != null) {
            result.setProductID(newProduct.getProductID());
            result.setName(newProduct.getName());
            result.setDescription(newProduct.getDescription());
            result.setPrice(newProduct.getPrice());
            result.setCategoryID(newProduct.getCategoryID());
            result.setImageURL(newProduct.getImageURL());
            result.setDiscountPrice(newProduct.getDiscountPrice());
